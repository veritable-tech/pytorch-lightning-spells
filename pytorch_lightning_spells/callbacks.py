...
class RandomAugmentationChoiceCallback(Callback):
    ...
    def on_train_batch_start(self, trainer, pl_module, batch, batch_idx):
        ...
        if self.no_op_prob and np.random.random() < self.no_op_prob:
            return
        self.get_callback().on_train_batch_start(trainer, pl_module, batch, batch_idx)
...
class SnapMixCallback(Callback):
    ...
    def on_train_batch_start(self, trainer, pl_module, batch, batch_idx):
        old_batch = batch
        batch, targets = batch
        target_activation_map = get_spm(batch, targets, self._model, self.image_size, self._half)
        bs = batch.size(0)
        ...
        while area_1 <= 0 or area_2 <= 0:
            if self.minmax:
                bby1_1, bby2_1, bbx1_1, bbx2_1 = rand_bbox_minmax(batch.size(), self.minmax)
                bby1_2, bby2_2, bbx1_2, bbx2_2 = rand_bbox_minmax(batch.size(), self.minmax)
            else:
        ...
        if self.cutmix_bbox is False:
            cropped = F.interpolate(
                cropped, size=(bby2_1 - bby1_1, bbx2_1 - bbx1_1), mode="bilinear", align_corners=True
            )
        batch[:, :, bby1_1:bby2_1, bbx1_1:bbx2_1] = cropped
        lamb_1 = 1 - target_activation_map[:, bby1_1:bby2_1, bbx1_1:bbx2_1].sum(1).sum(1)
        lamb_2 = target_activation_map_b[:, bby1_2:bby2_2, bbx1_2:bbx2_2].sum(1).sum(1)

        # Fall back to Cutmix lambda
        lamb_cutmix = 1 - ((bbx2_1 - bbx1_1) * (bby2_1 - bby1_1) / (batch.size(2) * batch.size(3)))
        lamb_1[torch.isnan(lamb_1)] = lamb_cutmix
        lamb_2[torch.isnan(lamb_2)] = 1 - lamb_cutmix
        # Combine targets
        new_targets = torch.stack(
            [targets.float(), targets[rand_index].float(), lamb_1.to(targets.device), lamb_2.to(targets.device)], dim=1
        )
        old_batch[0] = batch
...
class CutMixCallback(Callback):
    ...
    def on_train_batch_start(self, trainer, pl_module, batch, batch_idx):
        ...
        lambd_tensor = (
            batch.new(lambd).view(-1, *[1 for _ in range(len(targets.size()) - 1)]).expand(-1, *targets.shape[1:])
        )
        # Combine targets
        if self.softmax_target:
            new_targets = torch.stack([targets.float(), targets.flip(0).float(), lambd_tensor], dim=1)
        else:
            new_targets = targets * lambd_tensor + targets.flip(0) * (1 - lambd_tensor)
        old_batch[0] = batch
        old_batch[1] = new_targets
...
class MixUpCallback(Callback):
    ...
    def on_train_batch_start(self, trainer, pl_module, batch, batch_idx):
        ...
        batch_flipped = batch.flip(0).clone()
        lambd = np.random.beta(self.alpha, self.alpha, batch.size(0))
        lambd = np.concatenate([lambd[:, np.newaxis], 1 - lambd[:, np.newaxis]], axis=1).max(axis=1)
        # Create the tensor and expand (for batch inputs)
        lambd_tensor = (
            batch.new(lambd).view(-1, *[1 for _ in range(len(batch.size()) - 1)]).expand(-1, *batch.shape[1:])
        )
        # Combine input batch
        new_batch = batch * lambd_tensor + batch_flipped * (1 - lambd_tensor)
        # Create the tensor and expand (for target)
        lambd_tensor = (
            batch.new(lambd).view(-1, *[1 for _ in range(len(targets.size()) - 1)]).expand(-1, *targets.shape[1:])
        )
...
class TelegramCallback(Callback):
    ...
    def on_train_start(self, trainer: pl.Trainer, pl_module: pl.LightningModule):
        ...
        contents = [
            ...
            "Starting date: %s" % self.start_time.strftime(TelegramCallback.DATE_FORMAT),
        ]
        text = "\n".join(contents)
    ...
    def on_train_end(self, trainer: pl.Trainer, pl_module: pl.LightningModule):
        ...
        contents = [
            ...
            "Machine name: %s" % self.host_name,
            "Starting date: %s" % self.start_time.strftime(TelegramCallback.DATE_FORMAT),
            "End date: %s" % end_time.strftime(TelegramCallback.DATE_FORMAT),
